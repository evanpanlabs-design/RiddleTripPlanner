...
// apply_plan 分批缓冲（大行程输出截断对策）：按段号幂等存稿 → 齐后按序合并；打回后局部重提；total 变重置
{
  const T = "_test_batch_trip";
  const r1 = pushPlanBatch(T, 1, 3, { days: 7, events: [{ tmp_id: "e1" }, { tmp_id: "e2" }] });
  ok("final" in r1 && !r1.final && r1.count === 2 && r1.missing.join() === "2,3", "batch：第 1 段进缓冲不落地", JSON.stringify(r1));
  const r2 = pushPlanBatch(T, 3, 3, { days: 7, events: [{ tmp_id: "e4" }], checklist: [{ title: "c1" }] });
  ok("final" in r2 && !r2.final && r2.missing.join() === "2", "batch：乱序到段，报缺第 2 段", JSON.stringify(r2));
  const r3 = pushPlanBatch(T, 2, 3, { days: 7, events: [{ tmp_id: "e3" }] });
  ok("final" in r3 && r3.final && r3.merged.events.map((e: any) => e.tmp_id).join() === "e1,e2,e3,e4" && r3.merged.checklist.length === 1 && r3.merged.days === 7, "batch：齐后按段序合并整树", JSON.stringify(r3));
  const r4 = pushPlanBatch(T, 2, 3, { days: 7, events: [{ tmp_id: "e3fix" }, { tmp_id: "e3b" }] });
  ok("final" in r4 && r4.final && r4.merged.events.map((e: any) => e.tmp_id).join() === "e1,e2,e3fix,e3b,e4", "batch：打回后只重提第 2 段=覆盖，其余段保留", JSON.stringify(r4));
  const r5 = pushPlanBatch(T, 0, 3, { days: 7, events: [] });
  ok("error" in r5, "batch：非法 index 报错", JSON.stringify(r5));
  const r6 = pushPlanBatch(T, 1, 2, { days: 3, events: [{ tmp_id: "x" }] });
  ok("final" in r6 && !r6.final && r6.missing.join() === "2", "batch：total 变了重置残留缓冲", JSON.stringify(r6));
